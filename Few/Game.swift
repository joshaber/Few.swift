...
func renderForm(state: GameState) -> Element<GameState> {
	...
	let count = Label<GameState>(text: "\(state.count)")
			 |> sizeToFit
			 |> offset(0, 20)
...
func renderBackground(state: GameState) -> Element<GameState> {
	var element: Element<GameState> = empty()
	if state.count < 0 {
